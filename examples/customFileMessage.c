...
static void printHelp(const char * progName)
{
    ...
    printf("  -f, --filepath   path to file, relative to where this script runs (mandatory)\n");
...
{
    memset(moBuffer, 0, MO_SIZE);
    char * fileDataBuffer = moBuffer;
    ...
    if(fileDataBuffer)
    {
        ...
        fread(fileDataBuffer, 1, size, file);
        fclose(file);
        *fileSize = size;
    }
...
int main(int argc, char * argv[])
{
    ...
    while ((opt = getopt_long(argc, argv, "d:t:f:h", _longOptions, NULL)) != -1)
    {
        switch (opt)
        {
            case 'd':
            ...
            case 'f':
                strncpy(_filepath, optarg, PATH_MAX -1);
            break;
    ...
    if ((rVal == SUCCESS) && (gotArgs == true))
    {
        ...
        if(rbBegin(_serialDevice))
        {
            ...
            else
            {
                ...
                if(rbEnd())
                {
                    printf("Failed to find file, ending connection\r\n");
                    printf("Ended connection successfully\r\n");
                    rVal = FAILED_TO_FIND_FILE;
                    _run = false;
                }
                else
                {
